...
if (empty($_POST['firstname']) || empty($_POST['lastname']) || empty($_POST['item']) || empty($_POST['quantity']) || empty($_POST['date_added'])) {
...
if (mysqli_num_rows($result) > 0) {
    ...
    if ($currentAmount >= $quantity) {
        ...
        $insertQuery = "INSERT INTO Stock_Import (username, ItemName, Amount, DateImport) 
                        VALUES ('$username', '$item', '$quantity', '$dateImport')";
        ...
        header("Location: importItem.php?username=" . urlencode($username) . "&error=not_enough_stock");
        exit;
    }
} else {
